...
class CausalDiscoveryBase:
    ...
    def _partial_corr_independence_test(self, data, x, y, covar):
        ...
        self.tests_done.append(
            (
                self.cols[x],
                self.cols[y],
                {self.cols[i] for i in covar},
                p_val > self.alpha,
                p_val,
            )
        )
        self.tests_done_by_size[len(covar)].append(
            (
                self.cols[x],
                self.cols[y],
                {self.cols[i] for i in covar},
...
class SequentialLocalCausalDiscoveryBase(CausalDiscoveryBase):
    ...
    def run(self, data):
        ...
        def get_parents_using_nbrs(skeleton, sep_set):
            ...
            for i, j in combinations(neighbors_X, 2):
                ...
                if tmt_id not in sep_set[i][j]:
                    self.log(
                        "Marking parents: (%s, %s) | %s"
                        % (cols[i], cols[j], to_labels(sep_set[i][j]))
                    )
                    parents |= {i, j}
        ...
        data_matrix = pd.DataFrame(
            data=data.values, columns=range(data.values.shape[1])
        )
        ...
        while len(wait_queue) > 0:
            ...
            skeleton, sep_set = self.local_structure_learning_func(
                skeleton, sep_set, node_to_check, data_matrix
            )
            ...
            self.log(
                "Tests after MB(%s): %s"
                % (self.cols[node_to_check], str(self.ci_test_calls))
            )
            ...
            for i, j in combinations(list(skeleton.neighbors(tmt_id)), 2):
                if tmt_id in sep_set[i][j]:
                    self.log(
                        ...
                        % (self.cols[i], self.cols[tmt_id], self.cols[j])
                    )
                    non_colliders[i] |= {j}
                    non_colliders[j] |= {i}

            cpdag_sub = orient_colliders(
                skeleton.subgraph(nodes_done).to_directed(), sep_set, tmt=tmt_id
            )
