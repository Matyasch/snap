...
from ldecc_github.causal_discovery_base import SequentialLocalCausalDiscoveryBase
...
class SequentialDiscoveryAlgorithm(SequentialLocalCausalDiscoveryBase):
    ...
    def __init__(
        ...
        def local_structure_fn(g, sep_set, main_node_id, data_matrix):
            return self._find_local_structure(
                g,
                sep_set,
                main_node_id,
                data_matrix,
        ...
        super(SequentialDiscoveryAlgorithm, self).__init__(
    ...
    def _find_local_structure(
        self, init_graph, sep_set_init, node, data_matrix, indep_test_func
    ):
        nx = networkx
        ...
        g = (
            ...
            else self._create_complete_graph(node_ids)
        )
        # g = g.copy()
        ...
        while True:
            ...
            for j in node_ids:
                ...
                if len(adj_i) >= l:
                    adj_i_sorted = sorted(
                        adj_i, key=lambda n: 0 if n == self.tmt_id else 1
                    )
