...
from ldecc_github.utils import *
from ldecc_github.causal_discovery_base import CausalDiscoveryBase
...
class LDECCAlgorithm(CausalDiscoveryBase):
    ...
    def __init__(
        ...
        super(LDECCAlgorithm, self).__init__(
    ...
    def run(self, data):
        ...
        cols = data.columns
        self.cols = cols
        return self._run_ldecc(
            self._partial_corr_independence_test,
    ...
    def _run_ldecc(
        self, indep_test_func, data_matrix, alpha, treatment_node_id, outcome_node_id
    ):
            ...
            for n in to_be_oriented:
                for nA in spouses_tmt:
                    ...
                    if mark_as_child:
                        tmt_children |= {n}
                        self.log(
                            "Collider detected: (%s -> %s <- %s) | %s"
                            % (
                                self.cols[tmt],
                                self.cols[n],
                                self.cols[nA],
                                str(to_labels(cond_set)),
                            )
                        )
                        break
        ...
        def eager_collider_check(g, i, j, sep_set, tmt_neighbors):
            ...
            is_indep, p_val = is_independent(
                i, j, ({tmt} | sep_set[i][j]), ret_p_val=True
            )
            if not is_indep:
                c = self.cols
                self.log(
        ...
        node_ids = set(range(data_matrix.shape[1])) - set(self.ignore)
        ...
        g, sep_set, markov_blanket = find_markov_blanket(
            g, sep_set, node_ids, treatment_node_id
        )
        ...
        while True:
            ...
            node_ids_sorted = sorted(
                node_ids, key=lambda n: 0 if n in tmt_neighbors else 1
            )
                ...
                if len(adj_i) >= l:
                    ...
                    for k in combinations(adj_i_sorted, l):
                        ...
                        if is_indep:
                            ...
                            self.log(
                                "Removed edge (%s - %s) | %s; p_val=%f"
                                % (
                                    self.cols[i],
                                    self.cols[j],
                                    str(to_labels(sep_set[i][j])),
                                    p_val,
                                )
                            )
                            ...
                            if mark_non_collider or len(detected_parents) > 0:
                                ...
                                tmt_children |= get_non_collider_nodes(
                                    tmt_parents, non_colliders
                                )
                                to_be_oriented -= detected_parents | tmt_children

                                self.log(
                                    "Parents: %s"
                                    % str(set(self.cols[o] for o in tmt_parents))
                                )
                                self.log(
                                    "Children: %s"
                                    % str(set(self.cols[o] for o in tmt_children))
                                )
